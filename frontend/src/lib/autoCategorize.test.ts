...
describe('guessCategory', () => {
  ...
  it('matches a BTC code', () => {
    expect(guessCategory(tx({ raw: { bank_transaction_code: { description: 'GROCERY STORES' } } }))).toBe(132)
  })
  ...
  it('returns "other income" for incoming payments labelled innbetaling', () => {
    expect(guessCategory(tx({ creditDebit: 'CRDT', amount: 500, description: 'Innbetaling lønn' }))).toBe(113)
  })
  ...
  it('ignores ambiguous FINANCIAL INST codes', () => {
    expect(guessCategory(tx({ raw: { bank_transaction_code: { description: 'FINANCIAL INST' } } }))).toBeUndefined()
  })
})
