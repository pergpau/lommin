...
// A transaction is (re-)tagged as a transfer only when it is uncategorized. An existing
// category — manual or auto — is never overwritten, so a hand-picked category survives
// re-sync.
export function shouldTagAsTransfer(categoryId: number | undefined): boolean {
...
export async function tagTransferCategory(transferPairs: Map<string, string>): Promise<void> {
  ...
  const tx = d.transaction("transactions", "readwrite");
  for (const t of all) {
    if (!transferPairs.has(t.id)) continue;
    if (shouldTagAsTransfer(t.categoryId)) {
