...
export async function listBanks(country: string): Promise<BankEntry[]> {
  ...
  return asArray(data.aspsps ?? [], 'aspsps')
    .filter(isRecord)
    .map((b) => ({ name: optString(b.name) ?? '', country: optString(b.country) ?? '', logo: optString(b.logo) }))
...
export async function createSession(code: string): Promise<SessionResult> {
  ...
  return {
    ...
    accounts: asArray(data.accounts ?? [], 'accounts').map((item, i) => {
      ...
      const name = optString(a.details) ?? optString(a.product) ?? optString(a.account_servicer_reference)
...
export async function fetchTransactions(
  ...
  const res = await proxyFetch(`/accounts/${encodeURIComponent(accountUid)}/transactions?${params}`)
  const data = asRecord(await res.json(), 'transactions-respons')

  const txAccountUid = localUid ?? accountUid
  const raws = asArray(data.transactions ?? [], 'transactions').filter(isRecord) as RawTransaction[]
...
export async function fetchBalance(accountUid: string): Promise<number | undefined> {
  ...
  const preferred = balances.find((b) =>
    ['CLBD', 'XPCD', 'CLAV'].includes(b.balance_type ?? ''),
  ) ?? balances[0]
...
export async function fetchAllTransactions(
  ...
  do {
    const { transactions, continuationKey } = await fetchTransactions(accountUid, dateFrom, ck, localUid)
