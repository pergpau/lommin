...
export default function MonthlyChart({ bars, activeKey, onSelect, mode, onModeChange }: MonthlyChartProps) {
  ...
  const avgSaving = bars.length > 0 ? bars.reduce((s, b) => s + (b.saving ?? 0), 0) / bars.length : 0
  const maxBarVal = Math.max(...visibleBars.map((b) => Math.max(b.income, b.expenses, b.saving ?? 0)), 1)
  ...
  const renderBar = (b: MonthBar) => {
    ...
    const savingH = (b.saving ?? 0) > 0 ? Math.max(2, Math.round(((b.saving ?? 0) / maxBarVal) * BAR_MAX)) : 0
    return (
      <button
        key={b.key}
      ...
      >
        ...
        <div className={`text-[9px] font-medium mt-2 leading-tight text-center ${isActive ? 'text-text' : 'text-muted'}`}>
          {b.label}
        </div>
      </button>
    )
    ...
    <div className="card p-5">
      <div className="flex justify-between items-start mb-5">
        <div className="flex gap-6">
          <div>
            ...
            <div className="text-2xl font-semibold text-positive">{fmtAmount(selected?.income ?? 0, undefined, 0)}</div>
            <div className="text-xs text-muted mt-0.5">Snitt: {fmtAmount(avgIncome, undefined, 0)}</div>
          </div>
          {avgSaving > 0 && (
            <div>
              <div className="text-xs text-muted uppercase tracking-wider mb-1">Sparing</div>
              <div className="text-2xl font-semibold" style={{ color: '#7c3aed' }}>{fmtAmount(selected?.saving ?? 0, undefined, 0)}</div>
              <div className="text-xs text-muted mt-0.5">Snitt: {fmtAmount(avgSaving, undefined, 0)}</div>
            </div>
          )}
          <div>
            <div className="text-xs text-muted uppercase tracking-wider mb-1">Utgifter</div>
            <div className="text-2xl font-semibold text-negative">{fmtAmount(selected?.expenses ?? 0, undefined, 0)}</div>
            <div className="text-xs text-muted mt-0.5">Snitt: {fmtAmount(avgExpenses, undefined, 0)}</div>
          </div>
        </div>
      ...
      <div className="flex items-center gap-1">
        ...
        {mode === 'month' && (
          <button
            className="shrink-0 p-1 rounded text-muted hover:text-text disabled:opacity-20 disabled:cursor-default transition-colors"
            onClick={() => setWindowStart(Math.min(bars.length - PAGE_SIZE, windowStart + PAGE_SIZE))}
