...
function mainMeta(mainId: MainId): { icon: string; name: string; color: string } {
  if (mainId === 'uncategorized' || mainId === 'uncategorized-income') return { icon: '?', name: 'Ukategorisert', color: '#9ca3af' }
  const cat = MAIN_CATEGORY_MAP[mainId as number]
...
export default function SpendingBreakdown({ transactions, onCategoryChange }: Props) {
  ...
  const excludedPool = useMemo(
    () => transactions.filter(
      t => !t.isTransfer && t.categoryId != null && SUB_CATEGORY_MAP[t.categoryId]?.type === 'exclude',
    ),
    [transactions],
  )
            ...
            {subRows.map(({ subId, total }) => {
              ...
              return (
                <button
                  ...
                  onClick={() => setView({ level: 'txns', mainId, subId, ...(excluded ? { excluded: true } : {}) })}
                >
  ...
  return (
    <div className="flex flex-col gap-6">
      {sections.filter(s => s.rows.length > 0).map(({ label, rows, max, excluded }) => (
        <div key={label}>
          <h3 className="text-xs font-semibold uppercase tracking-wider text-muted mb-2 px-1">{label}</h3>
          <div className="card overflow-hidden">
            <div className="divide-y divide-border">
              {renderRows(rows, max, excluded)}
            </div>
          </div>
        </div>
      ))}
