...
export default function CategoryPicker({ currentCategoryId, onSelect, onClose }: CategoryPickerProps) {
  ...
  const [activeMainId, setActiveMainId] = useState<number | null>(currentSub?.mainCategoryId ?? null)

  const visibleMain = mainCategoriesForType(activeType)
  const activeMain = activeMainId != null ? visibleMain.find((m) => m.id === activeMainId) ?? null : null
    ...
    >
      <div className="bg-surface border border-border rounded-t-2xl sm:rounded-2xl w-full sm:max-w-lg h-[630px] max-h-[90vh] flex flex-col shadow-xl">
        ...
        <div className="flex items-center justify-between px-4 py-3 border-b border-border shrink-0">
          ...
          <button className="text-muted hover:text-text text-lg leading-none" onClick={onClose}>✕</button>
        </div>
        ...
        <div className="px-4 py-3 border-t border-border shrink-0 flex justify-between items-center">
          ...
          <button className="btn-ghost text-xs px-3 py-1.5" onClick={onClose}>Avbryt</button>
        </div>
      </div>
    </div>
  )
