    ...
    <div className="min-h-screen bg-bg text-text flex flex-col">
              ...
              <nav className="flex items-center gap-1">
                {nav.map(({ to, label }) => (
                  <Link
                    key={to}
                    to={to}
                    className={`px-3 py-1.5 rounded text-sm transition-colors ${pathname.startsWith(to)
      ...
      <footer className="border-t border-border py-4">
        <div className="max-w-3xl mx-auto px-4 flex items-center justify-center gap-4">
          <Link to="/privacy" className="text-xs text-muted hover:text-text transition-colors">Personvern</Link>
          <span className="text-border">·</span>
          <Link to="/terms" className="text-xs text-muted hover:text-text transition-colors">Vilkår</Link>
        </div>
      </footer>
    </div>
  )
