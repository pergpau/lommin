...
export default function AccountPage() {
  ...
  const months = useMemo(
    () => [...new Set(sorted.map((t) => txMonth(t.bookingDate ?? t.transactionDate)).filter(Boolean))].sort((a, b) => b.localeCompare(a)),
    [sorted],
  )

  const filtered = useMemo(
    () => (selectedMonth ? sorted.filter((t) => txMonth(t.bookingDate ?? t.transactionDate) === selectedMonth) : sorted),
  ...
  const resetSync = useCallback(async () => {
    if (!uid || !confirm('Slett lagrede transaksjoner for denne kontoen og nullstill synk-markøren? Neste synkronisering henter alt på nytt basert på historikkinnstillingen.')) return
  ...
  return (
    <div className="max-w-3xl mx-auto px-4 py-8">
      ...
      {(syncError || accountError) && <Alert type="error" message={accountError ?? syncError ?? ''} className="mb-4" />}
      {syncMsg && !syncing && !accountError && <Alert type="ok" message={syncMsg} className="mb-4" />}
