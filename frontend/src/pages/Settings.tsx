...
import { buildImportPayload, buildImportPayloadFromZip, parseSpiirCsvAccounts, parseSpiirZipAccounts, type SpiirAccount } from '../lib/spiirImport'
import { clearAccounts, clearTransactions, exportAll, getAccounts, importAll, type Account } from '../lib/store'
...
export default function Settings() {
  ...
  const saveFile = useCallback(async () => {
    if (passphrase.length < MIN_PASSPHRASE_LENGTH)
      return setMsg({ type: 'err', text: `Bruk minst ${MIN_PASSPHRASE_LENGTH} tegn for å beskytte sikkerhetskopien.` })
    setSyncing('save')
    setMsg(null)
    try {
  ...
  const loadFile = useCallback(async () => {
    if (!passphrase) return setMsg({ type: 'err', text: 'Skriv inn passordet du brukte ved lagring.' })
    setSyncing('load')
    setMsg(null)
    try {
  ...
  const wipeTransactions = useCallback(async () => {
    if (!confirm('Slett alle lagrede transaksjoner? Kontoene forblir tilkoblet; neste synkronisering henter dem på nytt.')) return
    setWiping(true)
    setMsg(null)
    try {
      await clearTransactions()
      setMsg({ type: 'ok', text: 'Transaksjoner slettet. Kjør Synkroniser på oversikten for å hente på nytt.' })
    } catch (e) {
      setMsg({ type: 'err', text: e instanceof Error ? e.message : 'Sletting feilet' })
    } finally {
      setWiping(false)
  ...
  const onSpiirFileChange = useCallback(async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (parsed.length === 0) {
      setSpiirMsg({ type: 'err', text: 'Fant ingen kontoer i filen. Sjekk at filen er en gyldig Spiir-eksport.' })
      return
    }
  ...
  const onSpiirZipChange = useCallback(async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      if (parsed.length === 0) {
        setSpiirMsg({ type: 'err', text: 'Fant ingen kontoer i ZIP-filen. Sjekk at filen er en gyldig Spiir dataeksport.' })
        return
      }
      const existing = await getAccounts()
      const initMap: Record<string, string> = {}
      ...
      setSpiirMsg({ type: 'err', text: err instanceof Error ? err.message : 'Kunne ikke lese ZIP-filen.' })
    } finally {
  ...
  const doSpiirImport = useCallback(async () => {
    ...
    try {
      ...
      setSpiirMsg({ type: 'ok', text: `Importerte ${inserted} transaksjoner fra ${spiirAccounts.length} konto${spiirAccounts.length !== 1 ? 'er' : ''}${skipNote}.` })
      setSpiirStep('idle')
    } catch (e) {
  ...
  return (
    <div className="max-w-2xl mx-auto px-4 py-8">
      ...
      {msg && <Alert type={msg.type === 'ok' ? 'ok' : 'error'} message={msg.text} className="mb-6" />}

      <Card className="p-5 mb-4">
        <h2 className="text-sm font-semibold text-text mb-1">CORS Proxy</h2>
        <p className="text-xs text-muted mb-3">
          Alle Enable Banking API-kall rutes gjennom denne proxyen. Deploy
          worker med <span className="mono text-text/70">wrangler deploy</span>.
        </p>
        <div className="mb-4 border border-warning/20 bg-warning/5 rounded-lg p-3">
          <p className="text-xs text-muted leading-relaxed">
            <span className="text-text/80 font-medium">Tillitsgrense:</span> den standard
            hostede proxyen videresender trafikken din og kan se transaksjonsdata og
            ditt kortlivede tilgangstoken (aldri signeringsnøkkelen). For fullt privat data,
            kjør din egen proxy og pek denne URL-en dit.
          </p>
        </div>
      ...
      <Card className="p-5 mb-4">
        ...
        <p className="text-xs text-muted mb-4">
          Eksporter dataene dine til en AES-GCM-kryptert fil. Legg den i Dropbox/Drive for synkronisering på tvers av enheter.
          Passordet forlater aldri enheten.
        </p>
        <Input
          label="Passord"
          type="password"
        ...
        />
        <p className="text-xs text-muted mb-3">
          Minst {MIN_PASSPHRASE_LENGTH} tegn. Bruk en lang passfrase – den kan ikke gjenopprettes, og
          styrken på krypteringen avhenger helt av den.
        </p>
        <div className="flex gap-2">
          <Button
      ...
      <Card className="p-5 mb-4">
        ...
        <p className="text-xs text-muted mb-4">
          Importer historiske transaksjoner fra Spiir. Velg CSV-eksport for enkel import, eller ZIP-eksport (full dataeksport) for bedre data med kontonavn, bank og kategorier fra Spiir. Duplikater hoppes over.
        </p>

        {spiirMsg && (
          <Alert type={spiirMsg.type === 'ok' ? 'ok' : 'error'} message={spiirMsg.text} className="mb-3" />
        )}

        {spiirStep === 'idle' && (
          <>
            <input ref={spiirFileRef} type="file" accept=".csv" className="hidden" onChange={onSpiirFileChange} />
            <input ref={spiirZipRef} type="file" accept=".zip" className="hidden" onChange={onSpiirZipChange} />
            <div className="flex gap-2">
        ...
        {(spiirStep === 'mapping' || spiirStep === 'importing') && (
          <>
            ...
            <div className="flex gap-2">
              ...
              <Button variant="ghost" disabled={spiirStep === 'importing'} onClick={cancelSpiirImport}>
                Avbryt
              </Button>
            </div>
          </>
      ...
      <Card className="p-5 border-negative/10">
        ...
        <p className="text-xs text-muted mb-4">
          ...
          <span className="mono">.pem</span>-filen på nytt. Eksisterende transaksjonsdata påvirkes ikke.
        </p>
        <div className="flex gap-2">
