...
export default function Dashboard() {
  ...
  const { syncing, syncMsg, error, failedAccounts, syncingAccountUids, run: runSync } = useSyncState()
    ...
    <div className="max-w-3xl mx-auto px-4 py-8">
      <div className="flex items-center justify-between mb-8">
        <div>
          ...
          <p className="text-muted text-sm mt-0.5">
            {accounts.length} konto{accounts.length !== 1 ? 'er' : ''} · {transactions.length} transaksjoner
          </p>
        </div>
        <div className="flex items-center gap-2">
          <Link to="/connect" className="btn-ghost text-xs">+ Legg til konto</Link>
          <Button variant="ghost" onClick={() => { setSaveOpen(true); setSaveMsg(null); setTimeout(() => passphraseRef.current?.focus(), 50) }}>
      ...
      {error && (
        <Alert type="error" message={error} className="mb-6">
          {error.includes('401') && (
            <> · <Link to="/connect" className="underline underline-offset-2">Koble til på nytt</Link></>
          )}
        </Alert>
      )}
      ...
      {chartData.length > 0 && (
        <div className="mb-8">
          <MonthlyChart
            ...
            mode={chartMode}
            onModeChange={(m) => { setChartMode(m); setSelectedMonth(null) }}
          />
        </div>
      )}
      ...
      {tab === 'kontoer' && (
        accounts.length === 0 ? (
          <EmptyState message="Ingen kontoer tilkoblet ennå." className="mb-6">
            <Link to="/connect" className="btn-primary inline-flex">Koble til en bank →</Link>
          </EmptyState>
        ) : (
      ...
      {tab === 'transaksjoner' && (
        recent.length > 0 ? (
          <TransactionTable
            transactions={recent}
            title={selectedMonthBar ? `Transaksjoner — ${selectedMonthBar.label}` : 'Siste transaksjoner'}
            onCategoryChange={async (txId, catId) => { await setCategoryId(txId, catId); refresh() }}
          />
        ) : (
          <div className="card p-10 text-center text-muted text-sm">Ingen transaksjoner denne måneden.</div>
        )
      )}
      ...
      {saveOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40" onClick={() => setSaveOpen(false)}>
          <div className="card p-5 w-full max-w-sm mx-4" onClick={(e) => e.stopPropagation()}>
            ...
            <p className="text-xs text-muted mb-4">Dataene krypteres med AES-GCM før de lagres. Passordet forlater aldri enheten.</p>
            {saveMsg && (
              <Alert type={saveMsg.type === 'ok' ? 'ok' : 'error'} message={saveMsg.text} className="mb-3" />
            )}
            <input
              ref={passphraseRef}
              type="password"
            ...
            <div className="flex gap-2 justify-end">
              <Button variant="ghost" onClick={() => setSaveOpen(false)}>Avbryt</Button>
