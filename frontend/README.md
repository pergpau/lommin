...
No proxy URL is configured by default. Point **Settings → CORS Proxy** in the app at a locally running proxy (`wrangler dev` from `proxy/`) or your deployed Worker — see [`../proxy/README.md`](../proxy/README.md). You can also set `VITE_PROXY_URL` in `.env` to bake in a default at build time.
